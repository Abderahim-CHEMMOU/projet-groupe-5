...
const ChercheurList = () => {
  ...
  return (
    <div className="chercheur-list-container">
      <h1 className="chercheur-list-title">Liste des Chercheurs</h1>
      <input
        type="text"
        placeholder="Rechercher un chercheur"
        value={searchTerm}
        onChange={handleSearch}
        className="chercheur-list-search"
      />
      <ul className="chercheur-list">
        {chercheurs.map(chercheur => (
          <li key={chercheur.id} className="chercheur-list-item">
            {chercheur.nom} - {chercheur.specialite}
            <div className="chercheur-list-buttons">
              <button onClick={() => openModal(chercheur.id)} className="details-button">Voir Détails</button>
              <button onClick={() => navigate(`/chercheurs/${chercheur.id}/edit`)} className="modify-button">Modifier</button>
              <button onClick={() => handleDelete(chercheur.id)} className="delete-button">Supprimer</button>
            </div>
          </li>
        ))}
      </ul>
      ...
      {selectedChercheur && (
        <Modal isOpen={modalIsOpen} onRequestClose={closeModal} className="Modal" overlayClassName="Overlay">
          <h2 className="modal-title">Détails du Chercheur</h2>
          <p className="modal-content"><strong>Nom:</strong> {selectedChercheur.nom}</p>
          <p className="modal-content"><strong>Spécialité:</strong> {selectedChercheur.specialite}</p>
      ...
      {totalPages > 1 && (
        <Pagination
          ...
          totalPages={totalPages}
          onPageChange={handlePageChange}
          className="chercheur-list-pagination"
        />
      )}

      <Link to="/chercheurs/new" className="add-chercheur-link">
        <button className="add-chercheur-button">Ajouter un Chercheur</button>
      </Link>
    </div>
  );
};
